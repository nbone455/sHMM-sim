...
rownames(snap_four) <- c("1","2","3","4")
colnames(snap_four) <- c("1","2","3","4")

## six
snap_six <- matrix(c(NA,1,NA,NA,NA,NA,NA,NA,1,NA,NA,NA,NA,1,NA,1,NA,NA,NA,NA,1,NA,1,NA,NA,NA,NA,1,NA,NA,1,NA,NA,NA,1,NA),
                   nrow=6, ncol=6)
rownames(snap_six) <- c("1","2","3","4","5","6")
colnames(snap_six) <- c("1","2","3","4","5","6")
## eight
snap_eight <- matrix(c(NA,1,NA,NA,NA,NA,NA,NA,
...
ncol=8, nrow=8)
rownames(snap_eight) <- c("1","2","3","4","5","6","7","8")
colnames(snap_eight) <- c("1","2","3","4","5", "6","7","8")
## ten 
snap_10 <-  matrix(ncol = 10, nrow = 10,byrow = T, c(NA,NA,NA,NA,NA,NA,NA,NA,NA,1,
...
snap_12 <- matrix(ncol = 12, nrow = 12, byrow = T, c(NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,1,
...
snap_14 <- matrix(ncol = 14, nrow = 14, byrow = T, c(NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,1,
...
snap_16 <-  matrix(ncol = 16, nrow = 16,byrow = T, c(NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,1,
...
### so it is now possible to go from 1 to the extreme other state -- breaking the threshold, 
...
# stores all the delta AICs from all the runs 
all_aic <- matrix(nrow = 3, ncol = 7)

aic_df <- data.frame(matrix(ncol=4, nrow=nsim*4))
colnames(aic_df) <- c("model", "states", "samplesize", "AIC")
aic_df_test <- aic_df
everyting <- list()
...
for(i in 1:nsim){
  ...
  # real data is threshold still (looking to see if the snap models are preferred versus normal HMMs)
  ...
  # making data frames for each type of data
...
# Model runs
  ...
  # 4 states (two hidden states)
  
  ## 50 sp.

  ## not sure why after updating R I need to specify model = "ER" but alas 
  
  four50_corr <- corHMM(phy = tree50, data = th50_four_corr, rate.cat = 2, model = "ER", node.states = "none") 
  four50_thresh <- rayDISC(model = "ER", phy= tree50, data = th50_four, rate.mat = snap_four, root.p = root.obs(prop = prop(th50_four), 4), node.states = "none")
  ...
  four50_corr1 <- corHMM(phy = tree50, data = th50_four_corr, rate.cat = 2, rate.mat = ratemat4, ip = p4, node.states = "none") 
  ...
  four100_thresh <- rayDISC(model =  "ER", phy= tree100, data = th100_four, rate.mat = snap_four, root.p = root.obs(prop = prop(th100_four), 4), node.states = "none")
  ...
  four100_corr1 <- corHMM(phy = tree100, data = th100_four_corr, rate.cat = 2, rate.mat = ratemat4, ip = p4, node.states = "none") 
  ...
  four200_thresh <- rayDISC(model = "ER", phy = tree200, data = th200_four, rate.mat = snap_four, root.p = root.obs(prop = prop(th200_four), 4), node.states = "none")
  ...
  four200_corr1 <- corHMM(phy = tree200, data = th200_four_corr, rate.cat = 2, rate.mat = ratemat4, ip = p4, node.states = "none")
  ...
  six50_thresh <- rayDISC(model = "ER", phy= tree50, data = th50_six, rate.mat = snap_six, root.p =  root.obs(prop = prop(th50_six), 6), node.states = "none")
  
  n_state <- 3
  
  ratemat6 <- make_starting_values(six50_corr, six50_thresh, n_state)
  p6 <- ratemat6[!is.na(as.vector(ratemat6))]
  ratemat6[!is.na(ratemat6)] <- 1:length(ratemat6[!is.na(ratemat6)])
  ratemat_final6 <- ratemat6
  ratemat_final6[!is.na(ratemat6)] <- p6
  p6[p6 <= 1e-9] <- 1e-9
  # bayesian corHMM run 
  six50_corr1 <- corHMM(phy = tree50, data = th50_six_corr, rate.cat = 3, rate.mat = ratemat6, ip = p6, node.states =  "none") 
  ...
  six100_thresh <- rayDISC(model = "ER", phy= tree100, data = th100_six, rate.mat = snap_six, root.p = root.obs(prop = prop(th100_six), 6), node.states = "none")
  ...
  six100_corr1 <- corHMM(phy = tree100, data = th100_six_corr, rate.cat = 3, rate.mat = ratemat6, ip = p6, node.states = "none") 
  ...
  six200_thresh <- rayDISC(model = "ER", phy = tree200, data = th200_six, rate.mat = snap_six, root.p = root.obs(prop = prop(th200_six), 6), node.states = "none")
  ...
  six200_corr1 <- corHMM(phy = tree200, data = th200_six_corr, rate.cat = 3, rate.mat = ratemat6, ip = p6, node.states = "none") 
  ...
  eight50_thresh <- rayDISC(model = "ER", phy= tree50, data = th50_eight, rate.mat = snap_eight, root.p = root.obs(prop = prop(th50_eight), 8), node.states = "none")
  
  n_state <- 4
  ratemat8 <- make_starting_values(eight50_corr, eight50_thresh, n_state)
  p8 <- ratemat8[!is.na(as.vector(ratemat8))]
  ratemat8[!is.na(ratemat8)] <- 1:length(ratemat8[!is.na(ratemat8)])
  ratemat_final8 <- ratemat8
  ratemat_final8[!is.na(ratemat8)] <- p8
  p8[p8 <= 1e-9] <- 1e-9
  
  eight50_corr1 <- corHMM(phy = tree50, data = th50_eight_corr, rate.cat = 4, rate.mat = ratemat8, ip = p8, node.states = "none") 
  
  ## 100 sp
  
  eight100_corr <- corHMM(phy = tree100, data = th100_eight_corr, rate.cat = 4, model = "ER", node.states = "none")
  eight100_thresh <- rayDISC(model = "ER", phy= tree100, data = th100_eight, rate.mat = snap_eight, root.p = root.obs(prop = prop(th100_eight), 8), node.states = "none")
  ...
  eight100_corr1 <- corHMM(phy = tree100, data = th100_eight_corr, rate.cat = 4, rate.mat = ratemat8, ip = p8, node.states = "none") 
  ...
  eight200_thresh <- rayDISC(model = "ER", phy = tree200, data = th200_eight, rate.mat = snap_eight, root.p = root.obs(prop = prop(th200_eight), 8), node.states = "none")
  ...
  eight200_corr1 <- corHMM(phy = tree200, data = th200_four_corr, rate.cat = 4, rate.mat = ratemat8, ip = p8, node.states = "none") 
  ...
  ten50_thresh <- rayDISC(model = "ER", phy= tree50, data = th50_ten, rate.mat = snap_10, root.p = root.obs(prop = prop(th50_ten), 10), node.states = "none")
  
  
  ratemat10 <- make_starting_values(ten50_corr, ten50_thresh, n_state)
  p10 <- ratemat10[!is.na(as.vector(ratemat10))]
  ratemat10[!is.na(ratemat10)] <- 1:length(ratemat10[!is.na(ratemat10)])
  ratemat_final10 <- ratemat10
  ratemat_final10[!is.na(ratemat10)] <- p10
  p10[p10 <= 1e-9] <- 1e-9
  
  ten50_corr1 <- corHMM(phy = tree50, data = th50_ten_corr, rate.cat = 5, rate.mat = ratemat10, ip = p10, node.states = "none") 
  ...
  ten100_thresh <- rayDISC(model = "ER", phy= tree100, data = th100_ten, rate.mat = snap_10, root.p = root.obs(prop = prop(th100_ten), 10), node.states = "none")
  ...
  ten100_corr1 <- corHMM(phy = tree100, data = th100_ten_corr, rate.cat = 5, rate.mat = ratemat10, ip = p10, node.states = "none") 
  ...
  ten200_thresh <- rayDISC(model = "ER", phy = tree200, data = th200_ten, rate.mat = snap_10, root.p = root.obs(prop = prop(th200_ten), 10), node.states = "none")
  ...
  ten200_corr1 <- corHMM(phy = tree200, data = th200_ten_corr, rate.cat = 5, rate.mat = ratemat10, ip = p10, node.states ="none") 
  ...
  twelve50_thresh <- rayDISC(model = "ER", phy= tree50, data = th50_12, rate.mat = snap_12, root.p = root.obs(prop = prop(th50_12), 12), node.states = "none")
  ...
  ratemat12[!is.na(ratemat12)] <- 1:length(ratemat12[!is.na(ratemat12)])
  ratemat_final12 <- ratemat12
  ratemat_final12[!is.na(ratemat12)] <- p12
  p12[p12 <= 1e-9] <- 1e-9
  
  #p16 <- as.vector(sixteen50_corr$solution)
  #p16 <- na.omit(p16)
  #ratemat16 <- sixteen50_corr$index.mat
  #ratemat16[!is.na(ratemat16)] <- 1:32
  #ratemat_final16 <- ratemat16
  #ratemat_final16[!is.na(ratemat16)] <- p16
  
 twelve50_corr1 <- corHMM(phy = tree50, data = th50_12_corr, rate.cat = 6, rate.mat = ratemat12, ip = p12, node.states = "none") 
 ...
 twelve100_thresh <- rayDISC(model = "ER", phy= tree100, data = th100_12, rate.mat = snap_12, root.p = root.obs(prop = prop(th100_12), 12), node.states = "none")
 ...
 twelve100_corr1 <- corHMM(phy = tree100, data = th100_12_corr, rate.cat = 6, rate.mat = ratemat12, ip = p12, node.states = "none") 
 ...
 twelve200_thresh <- rayDISC(model = "ER", phy= tree200, data = th200_12, rate.mat = snap_12, root.p = root.obs(prop = prop(th200_12), 12), node.states = "none")
 ...
 twelve200_corr1 <- corHMM(phy = tree200, data = th200_12_corr, rate.cat = 6, rate.mat = ratemat12, ip = p12, node.states = "none") 
 ...
 fourteen50_thresh <- rayDISC(model = "ER", phy= tree50, data = th50_14, rate.mat = snap_14, root.p = root.obs(prop = prop(th50_14), 14), node.states = "none")
 ...
 ratemat14[!is.na(ratemat14)] <- 1:length(ratemat14[!is.na(ratemat14)])
 ...
 fourteen50_corr1 <- corHMM(phy = tree50, data = th50_14_corr, rate.cat = 7, rate.mat = ratemat14, ip = p14, node.states = "none") 
 ...
 fourteen100_thresh <- rayDISC(model = "ER", phy= tree100, data = th100_14, rate.mat = snap_14, root.p = root.obs(prop = prop(th100_14), 14), node.states = "none")
 ...
 fourteen100_corr1 <- corHMM(phy = tree100, data = th100_14_corr, rate.cat = 7, rate.mat = ratemat14, ip = p14, node.states = "none") 
 ...
 fourteen200_thresh <- rayDISC(model = "ER", phy= tree200, data = th200_14, rate.mat = snap_14, root.p = root.obs(prop = prop(th200_14), 14), node.states = "none")
 ...
 p14[p14 <= 1e-9] <- 1e-9
  ...
  fourteen200_corr1 <- corHMM(phy = tree200, data = th200_14_corr, rate.cat = 7, rate.mat = ratemat14, ip = p14, node.states = "none") 
  ...
  #sixteen50_Mk <- rayDISC(phy = tree50, data = th50_16, rate.mat = mk_16, root.p = root.obs(prop = prop(th50_16), 16))
  ...
  ratemat16[!is.na(ratemat16)] <- 1:length(ratemat16[!is.na(ratemat16)])
  ratemat_final16 <- ratemat16
  ratemat_final16[!is.na(ratemat16)] <- p16
  p16[p16 <= 1e-9] <- 1e-9
  ...
  sixteen50_corr1 <- corHMM(phy = tree50, data = th50_16_corr, rate.cat = 8, rate.mat = ratemat16, ip = p16, node.states = "none") 
  
  
  ## 100
  # sixteen100_Mk <- rayDISC(phy= tree100, data = th100_16, rate.mat = mk_16, root.p = root.obs(prop = prop(th100_16), 16))
  ...
  #p16 <- as.vector(sixteen100_corr$solution)
  #p16 <- na.omit(p16)
  #ratemat16 <- sixteen100_corr$index.mat
  #ratemat16[!is.na(ratemat16)] <- 1:32
  #ratemat_final16 <- ratemat16
  #ratemat_final16[!is.na(ratemat16)] <- p16
  
  sixteen100_corr1 <- corHMM(phy = tree100, data = th100_16_corr, rate.cat = 8, rate.mat = ratemat16, ip = p16, node.states = "none") 
  
  
  ## 200
  #sixteen200_Mk <- rayDISC(phy = tree200, data = th200_16, rate.mat = mk_16, root.p=root.obs(prop = prop(th200_16), 16))
  ...
  #p16 <- as.vector(sixteen200_corr$solution)
  #p16 <- na.omit(p16)
  #ratemat16 <- sixteen200_corr$index.mat
  #ratemat16[!is.na(ratemat16)] <- 1:32
  #ratemat_final16 <- ratemat16
  #ratemat_final16[!is.na(ratemat16)] <- p16
  
  sixteen200_corr1 <- corHMM(phy = tree200, data = th200_16_corr, rate.cat = 8, rate.mat = ratemat16, ip = p16, node.states = "none") 
  ...
  aic_df_test[i,] <- put_df_aic(data = aic_df_test, fit = four50_thresh, i = 1)
  aic_df_test[i+1,] <- put_df_aic(data= aic_df_test, fit = four50_corr1, i = 1)
  
  aic_df_test[i+2,] <- put_df_aic(data = aic_df_test, fit = four100_thresh, i = 1)
  aic_df_test[i+3,] <- put_df_aic(data= aic_df_test, fit = four100_corr1, i = 1)
  
  aic_df_test[i+4,] <- put_df_aic(data = aic_df_test, fit = four200_thresh, i = 1)
  aic_df_test[i+5,] <- put_df_aic(data= aic_df_test, fit = four200_corr1, i = 1)
  
  ## six 
  
  aic_df_test[i+6,] <- put_df_aic(data = aic_df_test, fit = six50_thresh, i = 1)
  aic_df_test[i+7,] <- put_df_aic(data= aic_df_test, fit = six50_corr1, i = 1)
  
  aic_df_test[i+8,] <- put_df_aic(data = aic_df_test, fit = six100_thresh, i = 1)
  aic_df_test[i+9,] <- put_df_aic(data= aic_df_test, fit = six100_corr1, i = 1)
  
  aic_df_test[i+10,] <- put_df_aic(data = aic_df_test, fit = six200_thresh, i = 1)
  aic_df_test[i+11,] <- put_df_aic(data= aic_df_test, fit = six200_corr1, i = 1)
  
  ## eight 
  
  aic_df_test[i+12,] <- put_df_aic(data = aic_df_test, fit = eight50_thresh, i = 1)
  aic_df_test[i+13,] <- put_df_aic(data= aic_df_test, fit = eight50_corr1, i = 1)
  
  aic_df_test[i+14,] <- put_df_aic(data = aic_df_test, fit = eight100_thresh, i = 1)
  aic_df_test[i+15,] <- put_df_aic(data= aic_df_test, fit = eight100_corr1, i = 1)
  
  aic_df_test[i+16,] <- put_df_aic(data = aic_df_test, fit = eight200_thresh, i = 1)
  aic_df_test[i+17,] <- put_df_aic(data= aic_df_test, fit = eight200_corr1, i = 1)
  
  ## ten 
  
  aic_df_test[i+18,] <- put_df_aic(data = aic_df_test, fit = ten50_thresh, i = 1)
  aic_df_test[i+19,] <- put_df_aic(data= aic_df_test, fit = ten50_corr1, i = 1)
  
  aic_df_test[i+20,] <- put_df_aic(data = aic_df_test, fit = ten100_thresh, i = 1)
  aic_df_test[i+21,] <- put_df_aic(data= aic_df_test, fit = ten100_corr1, i = 1)
  
  aic_df_test[i+22,] <- put_df_aic(data = aic_df_test, fit = ten200_thresh, i = 1)
  aic_df_test[i+23,] <- put_df_aic(data= aic_df_test, fit = ten200_corr1, i = 1)
  
  ## twelve 
  
  aic_df_test[i+24,] <- put_df_aic(data = aic_df_test, fit = twelve50_thresh, i = 1)
  aic_df_test[i+25,] <- put_df_aic(data= aic_df_test, fit = twelve50_corr1, i = 1)
  
  aic_df_test[i+26,] <- put_df_aic(data = aic_df_test, fit = twelve100_thresh, i = 1)
  aic_df_test[i+27,] <- put_df_aic(data= aic_df_test, fit = twelve100_corr1, i = 1)
  
  aic_df_test[i+28,] <- put_df_aic(data = aic_df_test, fit = twelve200_thresh, i = 1)
  aic_df_test[i+29,] <- put_df_aic(data= aic_df_test, fit = twelve200_corr1, i = 1)
  
  ## fourteen 
  
  aic_df_test[i+30,] <- put_df_aic(data = aic_df_test, fit = fourteen50_thresh, i = 1)
  aic_df_test[i+31,] <- put_df_aic(data= aic_df_test, fit = fourteen50_corr1, i = 1)
  
  aic_df_test[i+32,] <- put_df_aic(data = aic_df_test, fit = fourteen100_thresh, i = 1)
  aic_df_test[i+33,] <- put_df_aic(data= aic_df_test, fit = fourteen100_corr1, i = 1)
  
  aic_df_test[i+34,] <- put_df_aic(data = aic_df_test, fit = fourteen200_thresh, i = 1)
  aic_df_test[i+35,] <- put_df_aic(data= aic_df_test, fit = fourteen200_corr1, i = 1)
  
  
  ## sixteen 
  
  aic_df_test[i+36,] <- put_df_aic(data = aic_df_test, fit = sixteen50_thresh, i = 1)
  aic_df_test[i+37,] <- put_df_aic(data= aic_df_test, fit = sixteen50_corr1, i = 1)
  
  aic_df_test[i+38,] <- put_df_aic(data = aic_df_test, fit = sixteen100_thresh, i = 1)
  aic_df_test[i+39,] <- put_df_aic(data= aic_df_test, fit = sixteen100_corr1, i = 1)
  
  aic_df_test[i+40,] <- put_df_aic(data = aic_df_test, fit = sixteen200_thresh, i = 1)
  aic_df_test[i+41,] <- put_df_aic(data= aic_df_test, fit = sixteen200_corr1, i = 1)
  
  everyting[[i]] <- aic_df_test
  
  
  #four AIC
  all_aic[1,1] <- delta50_four <- four50_thresh$AIC - four50_corr1$AIC
  all_aic[2,1] <- delta100_four <- four100_thresh$AIC - four100_corr1$AIC
  all_aic[3,1] <- delta200_four <- four200_thresh$AIC - four200_corr1$AIC
  
  #six AIC
  all_aic[1,2] <- delta50_six <- six50_thresh$AIC - six50_corr1$AIC
  all_aic[2,2] <- delta100_six <- six100_thresh$AIC - six100_corr1$AIC
  all_aic[3,2] <- delta200_six <- six200_thresh$AIC - six200_corr1$AIC
  
  ## deprecated but honestly, this part is a nightmare, it doesnt make any sense 
  
  #aic_df[i+2,1] <- "thresh"
  #aic_df[i+2,2] <- 4
  #aic_df[i+2,3] <- 50
  #aic_df[i+2,4] <- four50_thresh$AIC 
  
  #aic_df[i+1, 1] <- "mk"
  #aic_df[i+1,2] <- 4
  #aic_df[i+1,3] <- 50
  #aic_df[i+1,4] <- four50_thresh$AIC
  
  #eight AIC
  all_aic[1,3] <- delta50_eight <- eight50_thresh$AIC - eight50_corr1$AIC
  all_aic[2,3] <- delta100_eight <- eight100_thresh$AIC - eight100_corr1$AIC
  all_aic[3,3] <- delta200_eight <- eight200_thresh$AIC - eight200_corr1$AIC
  
  
  #ten AIC
  all_aic[1,4] <- delta50_ten <- ten50_thresh$AIC - ten50_corr1$AIC
  all_aic[2,4] <- delta100_ten <- ten100_thresh$AIC - ten100_corr1$AIC
  all_aic[3,4] <- delta200_ten <- ten200_thresh$AIC - ten200_corr1$AIC
  
  #twelve AIC
  all_aic[1,5] <- delta50_twelve <- twelve50_thresh$AIC - twelve50_corr1$AIC
  all_aic[2,5] <- delta100_twelve <- twelve100_thresh$AIC - twelve100_corr1$AIC
  all_aic[3,5] <- delta200_twelve <- twelve200_thresh$AIC - twelve200_corr1$AIC
  
  #fourteen AIC
  all_aic[1,6] <- delta50_fourteen <- fourteen50_thresh$AIC - fourteen50_corr1$AIC
  all_aic[2,6] <- delta100_fourteen <- fourteen100_thresh$AIC - fourteen100_corr1$AIC
  all_aic[3,6] <- delta200_fourteen <- fourteen200_thresh$AIC - fourteen200_corr1$AIC
  
  #sixteen AIC
  all_aic[1,7] <- delta50_sixteen <- sixteen50_thresh$AIC - sixteen50_corr1$AIC
  all_aic[2,7] <- delta100_sixteen <- sixteen100_thresh$AIC - sixteen100_corr1$AIC
  all_aic[3,7] <- delta200_sixteen <- sixteen200_thresh$AIC - sixteen200_corr1$AIC
  
  # SAVING EACH SIM
  sim_aic[[i]] <- all_aic
  
  #sim_bic[[count]] <- all_bic
  # returns from forea
}
...
saveRDS(all_sims, file = "heatmap4_aic_results.rds")
...
h4 = heatmap.2(x = avg_sims, labRow = c(50,100,500), labCol = c(4,6,8,10,12,14,16), xlab = "Hidden states", 
               main = "Heatmap 4: almost-threshold data, deltaAIC = threshAIC- corrAIC",
